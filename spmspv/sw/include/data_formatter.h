                                                        ...
                                                        bool skip_empty_rows)
{
    if (csr_matrix.num_rows % (pack_size * num_hbm_channels) != 0) {
        std::cout << "The number of rows of the sparse matrix should divide "
                  ...
                  << "Please use spmv_::io::util_round_csr_matrix_dim. "
                  << "Exit!" <<std::endl;
        exit(EXIT_FAILURE);
    }
    ...
    std::vector<MatrixPacketT, aligned_allocator<MatrixPacketT>> get_fused_matrix() {
        ...
        for (size_t i = 0, cnt = 0; i < partptr_packet_length; i++) {
            for (size_t j = 0; j < this->packet_size; j++, cnt++) {
                ...
                channel[i].indices.data[j] = this->formatted_adj_partptr[cnt] + meta_packet_length;
            }
            for (size_t j = 0; j < this->packet_size; j++, cnt++) {
                ...
                channel[i].vals.data[j] = tmp;
            }
        }

        for (size_t i = 0, cnt = 0; i < indptr_packet_length; i++) {
            for (size_t j = 0; j < this->packet_size; j++, cnt++) {
                channel[i + partptr_packet_length].indices.data[j] = this->formatted_adj_indptr[cnt];
            }
            for (size_t j = 0; j < this->packet_size; j++, cnt++) {
                DataT tmp; // store uint32 as raw bits to val_t (i.e. ap_ufixed)
                tmp(31,0) = ap_uint<32>(this->formatted_adj_indptr[cnt])(31,0);
                channel[i + partptr_packet_length].vals.data[j] = tmp;
            }
        }
        ...
        for (unsigned t = 0; t < formatted_matrix.num_row_partitions; t++) {
            ...
            for (unsigned p = 0; p < num_packets; p++) {
                ...
                for (unsigned k = 0; k < pack_size; k++) {
                    val_idx_packet.vals.data[k] = tile_val_buf[t][k + p * pack_size];
                    val_idx_packet.indices.data[k] = tile_idx_buf[t][k + p * pack_size];
                }
